...
export async function modifyFile(filePath: string, generatedCode: string, addinSummary?: string) {
  ...
  try {
    ...
    if (ulStart !== -1 && addinSummary) {
      const htmlIntroduction = `<p class="ms-font-l"> This is an add-in generated by Microsoft365 Agent in GitHub Copilot to ${addinSummary}</p>`;
      modifiedHtmlContent = htmlFileContent.replace(ulSection, htmlIntroduction);
    }

    const runStart = modifiedTsContent.indexOf('<b>Run</b>');
    const runEnd = runStart + '<b>Run</b>'.length;
    const runElement = modifiedTsContent.slice(runStart, runEnd);
